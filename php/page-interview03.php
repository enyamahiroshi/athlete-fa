<?php
$slag = $post->post_name;
get_header(); ?>

    <section class="mv-interview">
      <div class="mv-interview__main">
        <figure class="mv-interview__main__image">
          <img src="<?php echo get_stylesheet_directory_uri(); ?>/assets/images/interview/<?php echo $slag; ?>/img-mv.jpg" alt="" width="1248" height="659">
        </figure>
        <div class="mv-interview__main__data">
          <div class="mv-interview__main__data__semititle"><?php echo $slag; ?></div>
          <h1 class="mv-interview__main__catchcopy">
            <span>装置に命を吹き込むものづくり精神、</span><br><span>困難を乗り越えた先に喜びを実感</span>
          </h1>
          <div class="staff-data__meta">
            <div class="staff-data__name">Y.K.</div>
            <div class="staff-position">製造部 製造グループ</div>
            <div class="staff-join">2021年新卒入社</div>
          </div>
        </div>
      </div>
      <div class="staff-data__sub"><span>Since 2021</span><span>Manufacturing</span></div>
    </section>

    <?php //QA ?>
    <div class="qa-wrapper">
      <section class="sec qa qa-interview">
        <figure class="qa__image">
          <img class="parallax js-parallax" src="<?php echo get_stylesheet_directory_uri(); ?>/assets/images/interview/<?php echo $slag; ?>/img-qa01.jpg" alt="" width="960" height="640">
        </figure>
        <div class="qa__data">
          <h2 class="qa__data__q">入社した理由を教えてください。</h2>
          <strong class="qa__data__a-title">仕事での成長とプライベートの充実。どちらも大切にできる制度が整っています。</strong>
          <p class="qa__data__a__text">美しい自然を感じられる諏訪の地域が気に入り、この場所で働きたいと思いました。自分の意思で設計から営業までさまざまな経験ができるアスリートFAで技術を身に付け、自分自身を成長させられたらと思い入社を決めました。就職活動で重要視していたのはライフワークバランスです。年間休日が126日あり、その上、有給休暇もきちんと取れるという点がとても魅力的に感じました。また、企業見学の際に会社の雰囲気がとても良かったのが印象的でした。分からないことがあったら聞きやすい環境も魅力の一つでした。</p>
        </div>
      </section>
      <section class="sec qa qa-interview">
        <figure class="qa__image">
          <img class="parallax js-parallax" src="<?php echo get_stylesheet_directory_uri(); ?>/assets/images/interview/<?php echo $slag; ?>/img-qa02.jpg" alt="" width="960" height="640">
        </figure>
        <div class="qa__data">
          <h2 class="qa__data__q">仕事内容を教えてください。</h2>
          <strong class="qa__data__a-title">組み上がった機械体の精度を調整し、装置に命を吹き込みます。</strong>
          <p class="qa__data__a__text">製造部でメカニックを担当しています。メカ担当は、組み上がった装置の精度を調整し、実際に動かしてお客様が求める仕様を満たすか確認する部門です。“装置に命を吹き込む”大切なポジションだと思っています。ネジの種類、ユニットの組み立て具合、配管の間違いなどの不具合を見逃さないように気を配りながら、装置に愛情を持って作業に臨んでいます。だいたい一台の装置を4週間かけて調整しています。<br>入社当時に比べれば一人でできることも増えてきましたが、先輩の姿を見るとまだまだ力不足だという思いもあり、常に葛藤している感じです。「人にどう見られるかよりも、人をどう見るかを大切にする」「悩みは人を成長させてくれるもの」―。そんな上司の言葉を頭に置き、仕事に励んでいます。</p>
        </div>
      </section>
      <section class="sec qa qa-interview">
        <figure class="qa__image">
          <img class="parallax js-parallax" src="<?php echo get_stylesheet_directory_uri(); ?>/assets/images/interview/<?php echo $slag; ?>/img-qa03.jpg" alt="" width="960" height="640">
        </figure>
        <div class="qa__data">
          <h2 class="qa__data__q">仕事の面白さ、やりがい、これからの目標を教えてください。</h2>
          <strong class="qa__data__a-title">装置の旅立ちを見送りながら、仕事への達成感を噛みしめます。</strong>
          <p class="qa__data__a__text">装置の調整はμm（マイクロメートル＝1000分の1ミリ）単位で行う箇所もあり、とても細かい調整のため難易度が高く時間がかかってしまうこともありますが、社内の規格に収められた時は達成感を感じます。自分の担当した装置で製作された製品が身近な機器に使用されていることもあり、世の中の役に立っているという実感も大きいです。無事に社内検収に合格し、トラックに載せて装置の旅立ちを見送る時には、ひと仕事を終えたやりがいを感じます。<br>これからは装置にさらなる愛情を注ぐことで、装置に起きている不具合の早期発見、早期治療ができる調整役を目指したいと思います。また、装置調整のため、お客様の工場に入り、直接指導をする先輩の姿を見ると、自分もいつかは…と夢が膨らみます。</p>
        </div>
      </section>
    </div>

    <?php //Message ?>
    <section class="sec sec-small interview-message">
      <h2 class="message-title">入社を考えている方へ</h2>
      <p class="message-text">製造業とひとことで言っても、その種類は数多くあります。アスリートFAの装置はお客様毎のニーズ、仕様に沿って製作しますので、毎回同じ仕事内容になることはありません。納入先も国内海外様々で常に新鮮な気持ちでものづくりに取り組めています。毎回同じではないが故に難しい内容や初めての内容に躓くこともありますが、先輩方がフォローしてくれるのでとても心強いです。<br>また、仕事への取り組み方として、ただ受け身になって仕事をするのではなく、担当する装置に対して最適なスケジュールを立て、自分で順序を考えながら仕事を進めることができるので、時間管理能力も身につきます。<br>機械好きでいろいろなことに挑戦したいという人には自分の可能性を試せる良い会社だと思います。一緒に働けることを楽しみにしています。</p>
      <figure class="message-image">
        <img src="<?php echo get_stylesheet_directory_uri(); ?>/assets/images/interview/<?php echo $slag; ?>/img-message01.jpg" alt="" width="920" height="614">
      </figure>
    </section>

    <?php //その他のインタビュー ?>
    <section class="sec sec-full sec-other-interview">
      <h2 class="interview-title">その他のインタビュー</h2>
      <div class="staff-interview-wrapper">
        <?php //ループスライダー ?>
        <div class="loop-slider-wrap">
          <div class="loop-slider js-loop-slider">
            <div class="slider-item">
              <img src="<?php echo get_stylesheet_directory_uri(); ?>/assets/images/recruit/bg-other-interview.svg" alt="" width="4140" height="132">
            </div>
          </div>
        </div>
        <div class="staff-interview">
          <a href="<?php echo esc_url(home_url()); ?>/recruit/interview01/" class="staff-data">
            <figure class="staff-data__image">
              <div class="staff-data__photo">
                <img src="<?php echo get_stylesheet_directory_uri(); ?>/assets/images/recruit/staff/img-staff-card-image01.jpg" alt="" width="418" height="590">
              </div>
              <div class="staff-data__name">I.N.</div>
              <div class="staff-data__sub"><span>Since 2020</span><span>Design</span></div>
            </figure>
            <p class="staff-data__text">精密動作を可能とする機構設計、おもしろさと社会への貢献を実感</p>
            <div class="staff-data__meta">
              <div class="staff-position">設計部 設計グループ</div>
              <div class="staff-join">2020年新卒入社</div>
            </div>
          </a>
          <a href="<?php echo esc_url(home_url()); ?>/recruit/interview02/" class="staff-data">
            <figure class="staff-data__image">
              <div class="staff-data__photo">
                <img src="<?php echo get_stylesheet_directory_uri(); ?>/assets/images/recruit/staff/img-staff-card-image02.jpg" alt="" width="418" height="590">
              </div>
              <div class="staff-data__name">Y.W.</div>
              <div class="staff-data__sub"><span>Since 2021</span><span></span>System Control</div>
            </figure>
            <p class="staff-data__text">風通しのよい環境が魅力、仲間とともに頼れる技術者を目指す</p>
            <div class="staff-data__meta">
              <div class="staff-position">製造部 制御グループ</div>
              <div class="staff-join">2021年新卒入社</div>
            </div>
          </a>
          <?php /*
          <a href="<?php echo esc_url(home_url()); ?>/recruit/interview03/" class="staff-data">
            <figure class="staff-data__image">
              <div class="staff-data__photo">
                <img src="<?php echo get_stylesheet_directory_uri(); ?>/assets/images/recruit/staff/img-staff-card-image03.jpg" alt="" width="418" height="590">
              </div>
              <div class="staff-data__name">Y.K.</div>
              <div class="staff-data__sub"><span>Since 2021</span><span>Manufacturing</span></div>
            </figure>
            <p class="staff-data__text">装置に命を吹き込むものづくり精神、困難を乗り越えた先に喜びを実感</p>
            <div class="staff-data__meta">
              <div class="staff-position">製造部 製造グループ</div>
              <div class="staff-join">2021年新卒入社</div>
            </div>
          </a>
          */ ?>
        </div>
      </div>

      <div class="button-wrap --list">
        <a href="<?php echo esc_url(home_url()); ?>/recruit/new-graduates/" class="button-r-link-large button-circle-ani">
          <span>新卒採用ページへ戻る</span>
          <span class="svg-area">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" class="circle-ani">
              <circle cx="50%" cy="50%" r="28" class="circle-ani">
            </svg>
          </span>
        </a>
        <a href="<?php echo esc_url(home_url()); ?>/recruit/career/" class="button-r-link-large button-circle-ani">
          <span>キャリア採用ページへ戻る</span>
          <span class="svg-area">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" class="circle-ani">
              <circle cx="50%" cy="50%" r="28" class="circle-ani">
            </svg>
          </span>
        </a>
      </div>
    </section>
<?php get_footer("recruit"); ?>