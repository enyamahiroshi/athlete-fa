<?php get_header("english"); ?>
    ...
    $termCatTax = get_the_terms(get_the_ID(), 'products-en-category');
    ...
    <div class="has-column">
      <aside class="side-column side-column--products">
        <ul class="category-list position-nav js-positionNav">
          <li class="cat-item is-active"><a href="#features">Features</a></li>
          <li class="cat-item"><a href="#spec">Specification</a></li>
        </ul>
      </aside><?php //.side-column ?>
      <section class="main-column main-column--products-singular">
        <?php //特徴（ブロックエディタ） ?>
        <?php if($featuresTitle || $featuresText): ?>
        <h2 id="features" class="title05 js-positionNav-target">Features</h2>
        <div class="post-body">
...
<?php if($spec1Title || !empty($spec1Group[0]['spec-group1-label']) || $spec2Title || !empty($spec2Group[0]['spec-group2-label'])): ?>
        <h2 id="spec" class="title05 js-positionNav-target">Specification</h2>
...
<?php endif; ?>
    ...
    <?php //英語表記 ?>
    <?php
    $terms = get_the_terms($post->ID, 'products-en-category');
    if($terms):
