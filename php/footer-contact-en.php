...
<footer class="footer">
  <section class="footer__main">
    <div class="footer__main__company">
      ...
      <p class="footer__main__company__data">2970-1, Shiga, Suwa City, Nagano 392-0012, Japan</p>
      <p class="footer__main__company__data">
        TEL: <a href="tel:0266-53-3369">+81-266-53-3369</a><span>/</span>FAX: +81-266-58-1755
      </p>
    </div>
    <div class="footer__main__navigation">
      <section class="footer__main__navigation__main">
      <?php //カスタムメニューの呼び出し
          wp_nav_menu( array ( 'menu'=>'menu1en', 'container'=>'' , 'container_class' =>'' , 'menu_class'=>'menu',  'items_wrap'=>'<ul class="%2$s">%3$s</ul>'."\n" ) );
        ?>
        <?php //カスタムメニューの呼び出し
          wp_nav_menu( array ( 'menu'=>'menu2en', 'container'=>'' , 'container_class' =>'' , 'menu_class'=>'menu',  'items_wrap'=>'<ul class="%2$s">%3$s</ul>'."\n" ) );
        ?>
        <?php //カスタムメニューの呼び出し
          wp_nav_menu( array ( 'menu'=>'menu3en', 'container'=>'' , 'container_class' =>'' , 'menu_class'=>'menu',  'items_wrap'=>'<ul class="%2$s">%3$s</ul>'."\n" ) );
        ?>
      </section>
  ...
  <section class="footer__bottom">
    <a href="<?php echo esc_url(home_url()); ?>/en/privacy-policy/">Privacy Policy</a>
    <div class="copyright">©　2023　Athlete FA Corporation</div>
  </section>
</footer>
<?php wp_footer(); ?>
<script>
  //セレクトボックスの初期値をプレースホルダーのようにカスタマイズ
  (function ($) {
    ...
    $('input[type="file"]').on('change', function () {
        ...
        //text()で要素内のテキストを変更する
        $(this).parent('.custom-file').next('.custom-file-selected-file').text(file.name);
    });
  })(jQuery);
</script>
</body>
